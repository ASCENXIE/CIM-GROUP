...
def conv_pixel(pixel_data, kernel_weights):
    """
    计算一个像素的576个通道与64个卷积核的乘加结果。
    """
    pixel_data = pixel_data.astype(np.int32)
    kernel_weights = kernel_weights.astype(np.int32)

    results_total = np.zeros(64, dtype=np.int32)
    results_64 = np.zeros(64, dtype=np.int32)
    results_512 = np.zeros(64, dtype=np.int32)
...
def save_to_txt(pixel_data, kernel_weights, results_total, results_64, results_512):
    ...
    # === 保存像素数据 ===
    with open('C:/work_file/grade1/GROUP_RTL_CODE/python_verification/pixel_data_576x64.txt', 'w') as f:
        for val in pixel_data:
            f.write(f"{val & 0xFF:02X}\n")

    # === 保存卷积核数据 ===
    with open('C:/work_file/grade1/GROUP_RTL_CODE/python_verification/kernel_weights_576x64.txt', 'w') as f:
        for ch in range(kernel_weights.shape[1]):  # 遍历通道
            for k in range(kernel_weights.shape[0]):  # 遍历卷积核
    ...
    write_results_to_file('C:/work_file/grade1/GROUP_RTL_CODE/python_verification/conv_results_total_576x64.txt', results_total)
    write_results_to_file('C:/work_file/grade1/GROUP_RTL_CODE/python_verification/conv_results_576x64.txt', results_64)
    write_results_to_file('C:/work_file/grade1/GROUP_RTL_CODE/python_verification/conv_results_576x64.txt', results_512)
...
if __name__ == "__main__":
    ...
    pixel_data[:channel_size] = np.random.randint(-128, 127, size=channel_size, dtype=np.int32) 

    # === 生成卷积核权重（全部随机） ===
    kernel_weights = np.zeros((64, 576), dtype=np.int32)
    kernel_weights[:, :channel_size] = np.random.randint(-128, 127, size=(64, channel_size), dtype=np.int32)  
    ...
    # === 验证总结果正确性 ===
    check_full = np.zeros(64, dtype=np.int32)
    for i in range(64):
