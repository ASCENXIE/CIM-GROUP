...
module CIMD512X64NR//cimd macro 64
#(
...
generate
    ...
    begin
    if(g ==0)begin
        cimd_colgrp_nr #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(8),.MAC_WID(MAC_WID)) u_cimd_colgrp_nr512(
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
        .addr(addr),
        ...
        cimd_colgrp_nordy_nr #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(8),.MAC_WID(MAC_WID)) u_cimd_colgrp_nordy_nr512
        (
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
