...
module CIMD64X64NR//cimd macro 64
#(
parameter ROW_NUM=64,
parameter COL_NUM=64,
parameter PSUM_W=10,
parameter MAC_WID=22)
(
input CLK,
input MEB,
...
input [COL_NUM-1:0] DIN,
input [$clog2(ROW_NUM)-1:0]A,
input REN,
input [$clog2(ROW_NUM)-1:0]RA,// repair address

output [MAC_WID*8-1:0] MAC,
output SRDY,
...
ra=RA;
win= DIN;
din_inv=DIN;//in ROW64,only accept the lower 64bit
...
generate
    ...
    begin
    if(g ==0)begin
        cimd_colgrp_nr #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(1),.MAC_WID(MAC_WID)) u_cimd_colgrp_nr(
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
        .addr(addr),
        ...
        cimd_colgrp_nordy_nr #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(1),.MAC_WID(MAC_WID)) u_cimd_colgrp_nordy_nr
        (
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
...
cimd_repair_en_64 cimd_repair_en_64_inst(.CIMEN(cimen),.REN(ren),.RA(ra),.REPAIR_EN(repair_en));
