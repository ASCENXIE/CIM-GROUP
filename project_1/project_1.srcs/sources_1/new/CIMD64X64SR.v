...
module CIMD64X64SR//cimd macro 64
#(
...
generate
    ...
    begin
    if(g ==0)begin
        cimd_colgrp #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(1),.MAC_WID(MAC_WID)) u_cimd_colgrp(
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
        .addr(addr),
        ...
        cimd_colgrp_nordy #(.ROW_NUM(ROW_NUM),.PSUM_W(PSUM_W),.GROUP_NUM(1),.MAC_WID(MAC_WID)) u_cimd_colgrp_nordy
        (
        .clk(clk),
        .cimen(cimen),
        .wen(wen),
